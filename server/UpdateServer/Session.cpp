...
void update::server::Session::go()
{
	...
	boost::asio::spawn(socket_.get_io_service(), [this, self](boost::asio::yield_context yield)
	{
		try
		{
			...
			//get update_file_name from client
			...
			//send version
			auto ptr = std::make_shared<Update<void>>(update_file_name);
			auto version = ptr->getLastDisplayVersion();
			auto buffer_size = HEAD_SIZE + version.length();
			...
			setPacketRemain(buffer_ptr, version.c_str(), static_cast<uint16_t>(version.length()));
			...
			for (;;)
			{
				...
				else if (head[0] == 1 && head[1] == 0 && head[2] == 0 && head[3] == 3) //can update now,update with installPacket
				{
					...
					auto update_ptr = std::make_shared<Update<UpdateInstallPacket>>(update_file_name);
		...
		{
			auto error_message_size = strlen(e.what()) + 1;
			...
			setPacketRemain(buffer_ptr, e.what(), static_cast<uint16_t>(error_message_size));
			boost::system::error_code ec;
			boost::asio::async_write(socket_, boost::asio::buffer(buffer_ptr.get(), buffer_size), yield[ec]);
			if (ec)
				UPDATESERVER_WARN("send error message(%s) failed,%s", e.what(), ec.message().c_str());

			UPDATESERVER_ERROR("%s,coroutine exit", e.what());
		}
		catch (...)
		{
			auto error_message_size = strlen("unknow exception") + 1;
			auto buffer_size = HEAD_SIZE + error_message_size;
			auto buffer_ptr = std::shared_ptr<char>(new char[buffer_size], [](char*p) {delete[]p; });
			buffer_ptr.get()[0] = 1; buffer_ptr.get()[1] = 0; buffer_ptr.get()[2] = 0; buffer_ptr.get()[3] = 6;//error message
			setPacketRemain(buffer_ptr, "unknow exception", static_cast<uint16_t>(error_message_size));
			boost::system::error_code ec;
			boost::asio::async_write(socket_, boost::asio::buffer(buffer_ptr.get(), buffer_size), yield[ec]);
			if (ec)
				UPDATESERVER_WARN("send error message(unknow exception) failed,%s", ec.message().c_str());
